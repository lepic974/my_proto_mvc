...
if (is_uploaded_file($temp['tmp_name'])) {
  ...
  $dir = WEBROOTT . DS . 'monsite' . DS . 'webroot' . DS . 'img' . DS . $date;
...
$filetowrite = DS . 'monsite' . DS . 'webroot' . DS . 'img' . DS . $date.DS.$temp['name'];
 ...
 (EN) Serialization of image info to save the database */
  $data = array(
    'name' => $temp['name'],
    'file' =>  $temp_date[0].'/'.$temp_date[1].'/'.DS.$temp['name'],
    'type' => 'img'
  );
